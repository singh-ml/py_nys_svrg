...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 18 14:59:33 2021
...
def train_cifar(config, checkpoint_dir=None, data_dir=None):
    ...
    for epoch in range(10):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            #loss.backward(retain_graph=True)
            g=torch.autograd.grad(loss, net.parameters())[0]
            nel=torch.numel(g)
            H=torch.zeros(nel,nel)
            for i in range(nel):
                H[i]=torch.autograd.grad(g[i], net.parameters())[0].t()
             = torch.inv(H+0.01*torch.eye(nel))*g
            optimizer.step()
...
def main(num_samples=10, max_num_epochs=10, gpus_per_trial=2):
    data_dir = os.path.abspath("/home/singh/data")
    load_data(data_dir)
    config = {
